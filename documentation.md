# Creating and hosting interactive web maps
...
- QGIS3--See this tutorial for assistance installing on a Mac
- GitHub account
- GitHub Desktop
- Atom text editor
...
9. Type in a summary such as initial commit of images. If working with others on this project, add a more detailed description.
...
14. Click on an individual image.
15. Right click the image, and choose copy image address.
Either return to step 11 at a later point to manually copy and paste into the geospatial data table, or use the following steps:
16. If you used a spreadsheet in the previous step, open it. Otherwise create a New Excel or Google Sheets file.
...
20. In the empty cell type `=CONCATENATE`
...
1. Go to mymaps.google.com. If you do not have a Google account, create one to save projects.
...
6. At this point, it is possible to add new columns and include any text descriptions and data desired.
  ...
  - Add manual line break in a cell using shift + enter/return. Just using enter/return will select the next cell.
  - To add an image link use: `<img src="https://github.com/yourname/yourrep/img.jpg" alt="Don't forget to add descriptive alt text">`
...
## Working with QGIS
If downloading QGIS3 for the first time, see the appendix for installing on a Mac. It is not a one-click install and has very specific requirements.
...
9. From the top menu, click Web. Qgis2Web and QuickMapServices should both appear there. If not, quit QGIS and reopen it.
...
6. Export the layer.
7. Remove all layers from the map.
8. Add the reprojected layer that is in the git repository.
8. Click Web>QuickMapServices>OpenStreetMap>OSMStandard.
9. Do the points (or whatever data used) look correct on top of the OpenStreetMap basemap?
